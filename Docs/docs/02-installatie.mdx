...
Hier is een simpele visualisatie van de loop die achievements gebruikt om achievements te registreren en updaten.

```mermaid
graph TD
  A[setupAchievements] --> B[setReloadHandler]
  B --> C[ /achievements reload]
  C --> D[AchievementAPI triggerd de reloadHandler]
  D --> E[registerCustomAchievements wordt uitgevoerd]
  E --> F[achievements worden geüpdatet]
  F --> G[log complete]
  G --> C
```